...
class LayerDiscriminativeAnalyzer:
    ...
    def save_results(self, ranking: List[Tuple], output_path: str = None):
        ...
        if output_path is None:
            ...
            output_path = f"results/principled_layer_selection_results_multimodal_{model_name}.csv"
...
def main():
    ...
    print("="*100)
    print("PRINCIPLED LAYER SELECTION FOR JAILBREAK DETECTION (MULTIMODAL)")
    print("="*100)
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python principled_layer_selection_multimodal.py [qwen|llava]")
        sys.exit(1)
    ...
    try:
        ...
        print(f"✓ Detailed results saved to: principled_layer_selection_results_multimodal_{model_name}.csv")
...
def create_visualization(ranking: List[Tuple], composite_scores: Dict, analyzer=None):
    ...
    try:
        ...
        if analyzer:
            model_name = analyzer._get_model_name_for_filename()
            output_path = f"results/layer_selection_analysis_multimodal_{model_name}.png"
        else:
...
def create_second_figure_with_remaining_metrics(composite_scores, layers, remaining_metrics, analyzer=None):
    ...
    if analyzer:
        model_name = analyzer._get_model_name_for_filename()
        output_path2 = f"results/individual_metrics_rankings_multimodal_{model_name}.png"
    else:
